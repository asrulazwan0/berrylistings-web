...
export default function AdminDashboard() {
  ...
  const handleSave = async (data: Record<string, unknown>) => {
    setSaving(true);
    try {
      editing ? await updateProperty(editing.uuid, data) : await createProperty(data);
      setFormOpen(false);
      fetchAll();
  ...
  const handleDelete = async (uuid: string) => {
    if (deleting !== uuid) { setDeleting(uuid); return; }
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'flex-end', padding: 'var(--space-3) var(--space-5) 0', gap: 'var(--space-3)' }}>
      ...
      <main className="admin-main" id="main">
        <div className="stat-cards">
          ...
          <div className="card stat-card"><div className="stat-card__label">Value</div><div className="stat-card__value">{formatPrice(stats.totalValue)}</div></div>
        </div>
                    ...
                    <tr key={p.uuid}>
                      <td>
                        <div className="row-title-cell">
                          <div className="row-thumb" style={{ width: 48, height: 36, borderRadius: 4, overflow: 'hidden' }}>
                            {p.photos.length > 0 ? <img src={p.photos[0].url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (
                              <div className="photo-placeholder" aria-hidden="true" style={{ width: 48, height: 36 }}>
                                <svg width="16" height="16" viewBox="0 0 28 28" fill="none"><rect x="2" y="2" width="24" height="24" rx="3" stroke="currentColor" strokeWidth="1.5" /><circle cx="10" cy="10" r="2.5" stroke="currentColor" strokeWidth="1.5" /><path d="M2 20l7-7 5 5 4-4 8 8" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" /></svg>
                              </div>
                            )}
                          </div>
                          <div><strong>{p.title}</strong><span>{p.addressLine}, {p.city}</span></div>
                        </div>
                      </td>
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: 'var(--space-1)', justifyContent: 'flex-end' }}>
                          <button className="icon-btn" type="button" aria-label="Edit listing" onClick={() => handleEdit(p)}>
                          ...
                          <button className="icon-btn" type="button" aria-label={deleting === p.uuid ? 'Confirm delete' : 'Delete listing'} onClick={() => handleDelete(p.uuid)} style={deleting === p.uuid ? { color: 'var(--color-destructive)' } : undefined}>
